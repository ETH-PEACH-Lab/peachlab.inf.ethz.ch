...
export async function generateStaticParams() {
  const slugs = Object.keys(blogImports);
  // `output: export` requires at least one param, so keep a placeholder route
  // until the first blog post is added to blogImports.js.
...
export default async function BlogPostPage({ params }) {
  ...
  return (
    ...
    <div style={{ marginBottom: "20px" }}>
          <a href={`${basePath}/teaching/ucpi2026/`} style={{ color: "#666", textDecoration: "none", fontSize: "0.9rem", display: "flex", alignItems: "center" }}>
      ...
      <main className="blog-content">
              ...
              img: ({node, ...props}) => {
                ...
                if (src) {
                  ...
                  // Case 2: Relative path (not http, not /, or starts with ./)
                    ...
                    src = `${basePath}/assets/teaching/ucpi-blog-2026/blogs${relativePath}`;
                  }
                }
        ...
        <div style={{ marginTop: "60px", borderTop: "1px solid #eaeaea", paddingTop: "20px" }}>
          <ClientNote label={false} type="secondary">
            This blog post was written by <b>{authorName}</b>, a student of the UCPI 2026 seminar. 
          </ClientNote>
        </div>
      </main>
    </div>
